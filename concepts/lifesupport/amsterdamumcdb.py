...
def outliers_histogram(data, z_treshold=4.0, lower=None, upper=None, bins='auto'):
    ...
    Arguments:
        ...
        lower -- lower treshold for binning lower outliers together
        upper -- uppoer treshold for binning higer outliers together 
    """
...
def mad(data):
    ...
    Arguments:
        data -- the dataframe to calculate the median absolute deviatan for
    """
...
def calculate_z_range(data, z_thresh=3.5):
    """Calculate the range for the dataset base on the specified Z treshold: the number of standard 
    deviations from the median.

    Arguments:
        data -- the dataframe to calculate the median absolute deviation for.
        z_tresh -- the Z treshold.
    """
    MAD = mad(data)
    median = np.median(data)
